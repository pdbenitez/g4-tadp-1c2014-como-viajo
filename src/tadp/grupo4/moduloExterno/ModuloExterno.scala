...
object ModuloExterno {
  ...
  //var todasLasDirecciones = todosLosTransportes.flatMap(x=>x.listaDeParadas.map(y=> y.direccion)).toSet
  //Esta otra version es mas legible
  var todasLasDirecciones = (for{
    trans <- todosLosTransportes
    parada <- trans.listaDeParadas
  } yield parada.direccion).toSet
